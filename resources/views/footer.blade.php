<!-- footer section starts -->


<footer class="footersection p-3 mt-5" style="background:#fb5200" >
<div class="footer text-white text-center">
    <div class="row">
        <div class="col-lg-6">

            <p class="small mb-0 mt-1">&copy; copyright Hasan Bijoy</p>
        </div>
        <div class="col-lg-6 text-center ">
            <a class="text-light text text-decoration-none" href="#">Home</a>
            <a class="text-light" href=""><i class="fab fa-facebook"></i></a>
            <a class="text-light" href=""><i class="fab fa-whatsapp"></i></a>
            <a class="text-light" href=""><i class="fab fa-pinterest"></i></a>
            <a class="text-light" href=""><i class="fab fa-linkedin"></i></a>
            <a class="text-light" href=""><i class="fab fa-google-plus"></i></a>
            <a class="text-light" href=""><i class="fab fa-twitter"></i></a>
            <a class="text-light" href=""><i class="fab fa-discord"></i></a>
        </div>
    </div>
    </div>
</footer>

